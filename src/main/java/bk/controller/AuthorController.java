...
public class AuthorController {
    ...
    public @ResponseBody
    List<CoAuthorshipSimply> getTags(@RequestParam String id) {
        List<String> authors = coAuthorshipSimplyRepository.findByIdPrefix(id + "%");
        List<CoAuthorshipSimply> authorList = new ArrayList<>();
        for(String author: authors){
            CoAuthorshipSimply coAuthorshipSimply= new CoAuthorshipSimply();
            coAuthorshipSimply.setAuthorId1(author);
            authorList.add(coAuthorshipSimply);
        }
        return authorList;
    }
}
